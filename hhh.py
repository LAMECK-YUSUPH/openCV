...
def draw_perimeter(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        drawing_perimeter = False
        perimeter_points.append([(x, y)])  # Add the final point to the list

    elif event == cv2.EVENT_MOUSEMOVE and drawing_perimeter:
        perimeter_points[-1].append((x, y))
...
while True:
    ...
    drawing = frame.copy()
    for points in perimeter_points:
        for i in range(len(points) - 1):
            cv2.line(drawing, points[i], points[i + 1], (0, 255, 0), 2)
